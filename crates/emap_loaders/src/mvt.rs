...
impl EguiTileDrawable for MvtLayer {
    fn draw(&self, painter: &egui::Painter, rect: egui::Rect) {
        ...
        self.0.iter().for_each(|l|l.features.iter().for_each(|f|{
            match &f.geometry {
                rustitude_mvt::mvt::tile::Geometry::UnKnown => {},
                rustitude_mvt::mvt::tile::Geometry::Point { points } => {
                    points.iter().map(|p| Pos2::new((rect.min.x) + width * p.0,(rect.min.y) + height * p.1)).for_each(|p|{
                        // painter.circle_filled(p, 2.0, Color32::RED);
                        let name = f.props.get("name").map(|v|v.string_value()).unwrap_or_default();
                        painter.text(p, Align2::CENTER_CENTER, format!("{}\n{}",name, l.name),FontId {
                            size: 8.0,
