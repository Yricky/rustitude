...
pub mod tile {
    ...
    use crate::{mvt::{GeoCmd, ParameterInteger}, pb::vector_tile::tile::{self}};
    ...
    impl Layer {
        fn from(layer: tile::Layer) -> Self {
            ...
            Self{
                ...
                name: layer.name,
                features: layer.features.into_iter().map(|f|Feature::from(&keys,&values, extent,f) ).collect(),
            }
        }
    }
    ...
    impl Feature {
        fn from(keys: &Vec<String>,values: &Vec<Value>, extent: f32, feature: tile::Feature) -> Self {
            ...
            let geometry = match feature.r#type() {
                GeomType::Point => {
                    ...
                    let mut iter = (1 ..= (cmd.count() * 2)).map(|i| ParameterInteger(feature.geometry[i as usize]).value());
                    let mut vec = Vec::new();
            ...
            Self { id: feature.id(), geometry, props }
        }
    }
